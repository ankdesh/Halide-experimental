...
CodeGen_Accl::CodeGen_Accl(ostream &s, OutputKind output_kind, const std::string &guard) : IRPrinter(s), id("$$ BAD ID $$"), output_kind(output_kind), extern_c_open(false) {
    if (is_header()) {
...
void CodeGen_Accl::set_name_mangling_mode(NameMangling mode) {
...
string CodeGen_Accl::print_type(Type type, AppendSpaceIfNeeded space_option) {
    return type_to_c_type(type, space_option == AppendSpace);
}

string CodeGen_Accl::print_reinterpret(Type type, Expr e) {
    ostringstream oss;
    oss << "reinterpret<" << print_type(type) << ">(" << print_expr(e) << ")";
    return oss.str();
}

string CodeGen_Accl::print_name(const string &name) {
    ostringstream oss;
...
void CodeGen_Accl::compile(const Module &input) {
...
void CodeGen_Accl::compile(const LoweredFunc &f) {
...
void CodeGen_Accl::compile(const Buffer<> &buffer) {
...
void CodeGen_Accl::push_buffer(Type t, const std::string &buffer_name) {
...
void CodeGen_Accl::pop_buffer(const std::string &buffer_name) {
    allocations.pop(buffer_name);
}

string CodeGen_Accl::print_expr(Expr e) {
    id = "$$ BAD ID $$";
    e.accept(this);
    return id;
}

void CodeGen_Accl::print_stmt(Stmt s) {
    s.accept(this);
}

string CodeGen_Accl::print_assignment(Type t, const std::string &rhs) {
...
void CodeGen_Accl::close_scope(const std::string &comment) {
    cache.clear();
    indent--;
    do_indent();
    if (!comment.empty()) {
...
void CodeGen_Accl::visit(const Variable *op) {
    id = print_name(op->name);
}

void CodeGen_Accl::visit(const Cast *op) {
    print_assignment(op->type, "(" + print_type(op->type) + ")(" + print_expr(op->value) + ")");
}

void CodeGen_Accl::visit_binop(Type t, Expr a, Expr b, const char * op) {
    string sa = print_expr(a);
    string sb = print_expr(b);
    print_assignment(t, sa + " " + op + " " + sb);
}

void CodeGen_Accl::visit(const Add *op) {
    visit_binop(op->type, op->a, op->b, "+");
}

void CodeGen_Accl::visit(const Sub *op) {
    visit_binop(op->type, op->a, op->b, "-");
}

void CodeGen_Accl::visit(const Mul *op) {
    visit_binop(op->type, op->a, op->b, "*");
}

void CodeGen_Accl::visit(const Div *op) {
    int bits;
...
void CodeGen_Accl::visit(const Mod *op) {
    int bits;
    if (is_const_power_of_two_integer(op->b, &bits)) {
        ostringstream oss;
...
void CodeGen_Accl::visit(const Max *op) {
    print_expr(Call::make(op->type, "max", {op->a, op->b}, Call::Extern));
}

void CodeGen_Accl::visit(const Min *op) {
    print_expr(Call::make(op->type, "min", {op->a, op->b}, Call::Extern));
}

void CodeGen_Accl::visit(const EQ *op) {
    visit_binop(op->type, op->a, op->b, "==");
}

void CodeGen_Accl::visit(const NE *op) {
    visit_binop(op->type, op->a, op->b, "!=");
}

void CodeGen_Accl::visit(const LT *op) {
    visit_binop(op->type, op->a, op->b, "<");
}

void CodeGen_Accl::visit(const LE *op) {
    visit_binop(op->type, op->a, op->b, "<=");
}

void CodeGen_Accl::visit(const GT *op) {
    visit_binop(op->type, op->a, op->b, ">");
}

void CodeGen_Accl::visit(const GE *op) {
    visit_binop(op->type, op->a, op->b, ">=");
}

void CodeGen_Accl::visit(const Or *op) {
    visit_binop(op->type, op->a, op->b, "||");
}

void CodeGen_Accl::visit(const And *op) {
    visit_binop(op->type, op->a, op->b, "&&");
}

void CodeGen_Accl::visit(const Not *op) {
    print_assignment(op->type, "!(" + print_expr(op->a) + ")");
}

void CodeGen_Accl::visit(const IntImm *op) {
    if (op->type == Int(32)) {
...
void CodeGen_Accl::visit(const UIntImm *op) {
    print_assignment(op->type, "(" + print_type(op->type) + ")(" + std::to_string(op->value) + ")");
}

void CodeGen_Accl::visit(const StringImm *op) {
    ostringstream oss;
    oss << Expr(op);
    id = oss.str();
}
...
void CodeGen_Accl::visit(const FloatImm *op) {
    if (isnan(op->value)) {
        id = "nan_f32()";
...
void CodeGen_Accl::visit(const Call *op) {
...
void CodeGen_Accl::visit(const Load *op) {
...
void CodeGen_Accl::visit(const Store *op) {
...
void CodeGen_Accl::visit(const Let *op) {
    string id_value = print_expr(op->value);
...
void CodeGen_Accl::visit(const Select *op) {
    ostringstream rhs;
...
void CodeGen_Accl::visit(const LetStmt *op) {
    string id_value = print_expr(op->value);
    Expr new_var = Variable::make(op->value.type(), id_value);
...
void CodeGen_Accl::visit(const AssertStmt *op) {
...
void CodeGen_Accl::visit(const ProducerConsumer *op) {
    do_indent();
    if (op->is_producer) {
...
void CodeGen_Accl::visit(const For *op) {
...
void CodeGen_Accl::visit(const Provide *op) {
    internal_error << "Cannot emit Provide statements as C\n";
}

void CodeGen_Accl::visit(const Allocate *op) {
    open_scope();
...
void CodeGen_Accl::visit(const Free *op) {
...
void CodeGen_Accl::visit(const Realize *op) {
    internal_error << "Cannot emit realize statements to C\n";
}

void CodeGen_Accl::visit(const IfThenElse *op) {
...
void CodeGen_Accl::visit(const Evaluate *op) {
...
void CodeGen_Accl::visit(const Shuffle *op) {
    internal_error << "Cannot emit vector code to C\n";
}

void CodeGen_Accl::test() {
    ...
    {
        CodeGen_Accl cg(source, CodeGen_Accl::CImplementation);
        cg.compile(m);
    }
...
;
    ...
    std::cout << "CodeGen_Accl test passed\n";
}
